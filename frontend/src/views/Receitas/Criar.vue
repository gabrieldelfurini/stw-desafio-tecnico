      ...
      <div class="card-body">
        ...
        <div class="mb-3">
          ...
          >
            ...
            <option
              v-for="ingrediente in ingredientesList"
              :key="ingrediente.id"
        ...
        <div class="mb-3">
          <button type="button" @click="salvarReceita" class="btn btn-primary">
            Salvar
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    salvarReceita() {
      ...
      this.model.receita.ingredientesAdicionados =
        this.ingredientesSelecionados;

      var mythis = this;
      axios
        .post("http://127.0.0.1:8000/api/receitas", this.model.receita)
        .then((res) => {
    ...
    tirarIngredienteDaLista() {
      this.ingredientesList.forEach((ingredienteRequisicao) => {
        this.model.receita.ingredientesAdicionados.forEach(
          (ingredienteLista) => {
            if (ingredienteRequisicao.id === ingredienteLista.id) {
              let elementoRemovida = this.ingredientesList.splice(
                this.ingredientesList[ingredienteRequisicao.id],
                1
              );
            }
          }
    ...
    deletarIngrediente(ingredienteId) {
      ...
      for (
        let index = 0;
        index < this.model.receita.ingredientesAdicionados.length;
        index++
      ) {
        if (
          this.model.receita.ingredientesAdicionados[index].id === ingredienteId
        ) {
          retirado = this.model.receita.ingredientesAdicionados[index];
          this.model.receita.ingredientesAdicionados.splice(
            this.model.receita.ingredientesAdicionados[index],
            1
          );
        }
      }
